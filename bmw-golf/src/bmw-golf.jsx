...
const drawPairings = (flightA,flightB,flightC,numFlights) => {
  ...
  const remA=sA.slice(min), remBC=sBC.slice(min);
  for(let i=0;i<remA.length;i+=2) pairs.push(remA[i+1]?[remA[i],remA[i+1]]:[remA[i],'']);
  for(let i=0;i<remBC.length;i+=2) pairs.push(remBC[i+1]?[remBC[i],remBC[i+1]]:[remBC[i],'']);
  const groups=[];
...
const Logo = ({size=44}) => (
  <svg width={size} height={size} viewBox="0 0 48 48" fill="none">
    <polygon points="24,2 44,10 44,30 24,46 4,30 4,10" fill={C.green} stroke={C.goldL} strokeWidth="1.6"/>
    <polygon points="24,6 40,13 40,28 24,42 8,28 8,13" fill="none" stroke={C.goldL} strokeWidth="0.5" strokeOpacity="0.4"/>
    {[[24,12],[31,16],[31,26],[24,30],[17,26],[17,16]].map(([x,y],i)=>(
      <circle key={i} cx={x} cy={y} r="0.9" fill={C.goldL} fillOpacity="0.6"/>
    ))}
    <circle cx="24" cy="21" r="7.5" fill="none" stroke={C.goldL} strokeWidth="0.7" strokeOpacity="0.4"/>
    <circle cx="24" cy="21" r="1.4" fill={C.goldL} fillOpacity="0.8"/>
    <text x="24" y="23.5" textAnchor="middle" fill={C.goldL}
      style={{fontSize:'7.5px',fontWeight:900,fontFamily:"Georgia,serif",letterSpacing:'1.5px'}}>BMW</text>
    <text x="24" y="36.5" textAnchor="middle" fill={C.goldL}
      style={{fontSize:'3px',fontFamily:'sans-serif',letterSpacing:'1.4px',fontWeight:600}}>GOLF GROUP</text>
  </svg>
);
    ...
    <div style={{background:C.bg,minHeight:'100vh',fontFamily:"'DM Sans',sans-serif",display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      ...
      <Card style={{maxWidth:520,width:'100%',padding:36}}>
        <div style={{textAlign:'center',marginBottom:24}}><Logo size={72}/>
          <h1 style={{fontFamily:"'Playfair Display',serif",color:C.green,fontSize:'1.6rem',margin:'12px 0 4px'}}>BMW Golf Group</h1>
  ...
  if(loading) return (
    <div style={{background:C.bg,minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{textAlign:'center'}}>
        ...
        <div style={{color:C.muted,fontSize:'0.85rem'}}>Loading BMW Golf Group…</div>
      </div>
    </div>
  );
        ...
        <div style={{maxWidth:1060,margin:'0 auto',padding:'0 16px'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',height:56,gap:10}}>
            <div style={{display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
              <Logo size={36}/>
              <div>
                <div style={{fontFamily:"'Playfair Display',serif",color:C.green,fontSize:'0.95rem',lineHeight:1.1,fontWeight:700}}>BMW Golf Group</div>
